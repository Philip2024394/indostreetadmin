...
import { PencilIcon, TrashIcon, PlusCircleIcon, ClipboardListIcon, BookOpenIcon } from '../shared/Icons';
import SafeTradingGuide from './SafeTradingGuide';
import FoodMainPage from './directory/FoodMainPage';
import FoodDirectoryPage from './directory/FoodDirectoryPage';
...
type VendorView = 'management' | 'directory-main' | 'directory-list';
...
const VendorDashboard: React.FC<VendorDashboardProps> = ({ user, onLogout }) => {
  ...
  const [view, setView] = useState<VendorView>('management');
  ...
  const handleDirectoryNavigation = (targetView: 'directory') => {
    if (targetView === 'directory') {
      setView('directory-list');
    }
  };

  const navItems = [
    { view: 'management', label: 'My Items & Orders', icon: <ClipboardListIcon className="w-5 h-5 mr-3" /> },
    { view: 'directory-main', label: 'Public Food Directory', icon: <BookOpenIcon className="w-5 h-5 mr-3" /> },
  ];

  const getTitle = () => {
    switch(view) {
        case 'management': return user.profile.shopName || 'Vendor Dashboard';
        case 'directory-main': return 'Food Directory';
        case 'directory-list': return 'Explore Food Types';
  ...
  const renderContent = () => {
    switch (view) {
      case 'directory-main':
        return <FoodMainPage onNavigate={handleDirectoryNavigation} />;
      case 'directory-list':
        return <FoodDirectoryPage onBack={() => setView('directory-main')} />;
      case 'management':
      default:
        return (
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className="bg-white rounded-lg shadow-md">
                <div className="p-6 border-b flex justify-between items-center">
                  ...
                  <button onClick={() => handleOpenModal()} className="flex items-center px-4 py-2 bg-orange-500 text-white rounded-md hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500">
                  ...
                  <div className="divide-y divide-gray-200">
                    {items.map(item => (
                      <div key={item.id} className="p-4 flex items-center justify-between hover:bg-gray-50">
                        ...
                        <div className="flex items-center space-x-4">
                          <ToggleSwitch enabled={item.isAvailable} onChange={(enabled) => handleAvailabilityChange(item.id, enabled)} />
  ...
  return (
    <Layout 
      ...
      navItems={navItems}
      currentView={view}
      onViewChange={(newView) => setView(newView as VendorView)}
    >
       <div className="mb-6">
