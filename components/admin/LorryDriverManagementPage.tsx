...
import { PlusCircleIcon, PencilIcon, TrashIcon, SearchIcon, TruckIcon } from '../shared/Icons';
import ToggleSwitch from '../shared/ToggleSwitch';

const LorryDriverManagementPage: React.FC = () => {
    ...
    const filteredVehicles = useMemo(() => {
        return vehicles.filter(v => {
            if (v.type !== VehicleType.Lorry) return false;
        ...
        <>
            <div className="bg-white rounded-lg shadow-md">
                <div className="p-4 sm:p-6 border-b">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <h3 className="text-xl font-semibold text-gray-800">Lorry Driver Management</h3>
                            <p className="text-sm text-gray-500 mt-1">Manage all registered lorry drivers and vehicles.</p>
                        </div>
                        <button 
                ...
                <div className="overflow-x-auto">
                    {loading ? <p className="p-6 text-center text-gray-500">Loading lorry data...</p> : (
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Driver / Vehicle</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase hidden lg:table-cell">Service Type</th>
                             ...
                             <tbody className="bg-white divide-y divide-gray-200">
                                {filteredVehicles.length > 0 ? filteredVehicles.map(v => (
                                    ...
                                    <tr><td colSpan={4} className="text-center py-10 px-6 text-gray-500">No lorries found.</td></tr>
                                )}
                             </tbody>
                        </table>
                    )}
            ...
            {isFormOpen && (
                <VehicleFormModal 
                    ...
                    onSave={handleSave}
                    defaultVehicleType={editingVehicle ? undefined : VehicleType.Lorry}
                />
            )}
        </>
    );
...
export default LorryDriverManagementPage;